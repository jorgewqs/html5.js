...
;(function() {
  ...
  /** The pre-processed html5.js source */
  var source = preprocess(fs.readFileSync(path.join(__dirname, 'html5.js'), 'utf8'));
  ...
  function closureCompile(source, message, callback) {
    ...
    console.log(message == null ? 'Compressing html5.js using the Closure Compiler...' : message);
  ...
  function uglify(source, message, callback) {
    ...
    console.log(message == null ? 'Compressing html5.js using UglifyJS...' : message);
  ...
  function onUglifyGzip(exception, result) {
    ...
    var message = 'Compressing html5.js combining Closure Compiler and UglifyJS...';
  ...
  function onComplete() {
    ...
    fs.writeFileSync(path.join(distPath, 'html5.compiler.js'), compiled.source);
    fs.writeFileSync(path.join(distPath, 'html5.compiler.js.gz'), compiled.gzip);

    // save the Uglified version to disk
    fs.writeFileSync(path.join(distPath, 'html5.uglify.js'), uglified.source);
    fs.writeFileSync(path.join(distPath, 'html5.uglify.js.gz'), uglified.gzip);

    // save the hybrid minified version to disk
    fs.writeFileSync(path.join(distPath, 'html5.hybrid.js'), hybrid.source);
    fs.writeFileSync(path.join(distPath, 'html5.hybrid.js.gz'), hybrid.gzip);
    ...
    fs.writeFileSync(path.join(__dirname, 'html5.min.js'),
